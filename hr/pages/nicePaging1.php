...
class nicePaging{
	...
	public function createPaging($link){
		...
		$paging='<div class="pagination pagination-centered"><ul>';
		if($this->page>1){
			$paging.='<li><a href="'.$link.$this->separator.'pagenumber=1" title="First page">&lt;&lt;</a></li>';
			$paging.='<li><a href="'.$link.$this->separator.'pagenumber='.($this->page-1).'" title="Previous page">&lt;</a></li>';
		}
		
		if($start>$this->maxPages){
			$paging.='<li><a href="'.$link.$this->separator.'pagenumber='.($start-1).'" title="Page '.($start-1).'">...</a></li>';
		}
		
		for($i=$start;$i<=$end;$i++){
			if($this->page==$i){
				$paging.='<li><a href="" style="color:#000; background-color: #f5f5f5;">'.$i.'</a></li>';
			}
			else{
				$paging.='<li><a href="'.$link.$this->separator.'pagenumber='.$i.'" title="Page '.$i.'">'.$i.'</a></li>';
			}
		}
		
		if($end<$this->totalPages){
			$paging.='<li><a href="'.$link.$this->separator.'pagenumber='.($end+1).'" title="Page '.($end+1).'">...</a></li>';
		}
		
		if($this->page<$this->totalPages){
			$paging.='<li><a href="'.$link.$this->separator.'pagenumber='.($this->page+1).'" title="Next page">&gt;</a></li>';
			$paging.='<li><a href="'.$link.$this->separator.'pagenumber='.$this->totalPages.'" title="Last page">&gt;&gt;</a></li>';
		}
