...
$call = new globalm;
	  ...
	  if( isset( $_GET['id'] ) ) {
	       if( $call->isMSGdeleted( $_GET['id'] ) ) {
		       ...
		       $_SESSION['message'] = "Your message has been deleted successfully gdgdgdgdgdgdgdgdgddgdg.";
				 header("location:index.php?page=msg");
				 exit;
		   }
	  } else if( isset($_POST['del_mult']) ) {
	       if( $_POST['myCheckbox'] == null  ) {
		       $_SESSION['error'] = "<font color='#FF0000'>Please mark at least one checkbox and hit that button again.".$call->isMSGdeleted( $_GET['id'] );
		       header("location:index.php?page=msg");
			   exit;
		   } else {
		     $isError = false;
		    ...
		    if( !$isError ) {
			  ...
			  $_SESSION['error'] = "<font color='#FF0000'>An error occured. We are working on it. Please retry later.";
				header("location:admin_index.php?page=msg");
				exit;
			}
		}
